...
public:
	...
	Steinberg::Vst::SampleAccurate::Parameter sin_tone_parameter {parameter_id::sin_tone, 1.};
...
Steinberg::tresult PLUGIN_API sin_tone_processor::getState(Steinberg::IBStream* in_state) {
    ...
    IBStreamer streamer(in_state, kLittleEndian);
    streamer.writeDouble(sin_tone_parameter.getValue());
    return kResultTrue;
}
...
template <Steinberg::Vst::SymbolicSampleSizes SampleSize>
void sin_tone_processor::process(Steinberg::Vst::ProcessData& in_data) {
	Steinberg::Vst::ProcessDataSlicer slicer(8);

	auto do_processing = [this](Steinberg::Vst::ProcessData& data) {
		Steinberg::Vst::ParamValue sin_tone = sin_tone_parameter.advance(data.numSamples);
...
void sin_tone_processor::handle_parameter_changes(Steinberg::Vst::IParameterChanges* in_changes) {
	...
	for (auto i = 0; i < change_count; ++i) {
		if (auto queue = in_changes->getParameterData(i)) {
			...
			if (param_id == parameter_id::sin_tone) {
				sin_tone_parameter.beginChanges(queue);
			}
		}
	}
}
...
tresult PLUGIN_API sin_tone_processor::process(Steinberg::Vst::ProcessData& in_data) {
	state_transfer.accessTransferObject_rt(
		[this](const auto& state_model) { sin_tone_parameter.setValue(state_model.sin_tone); });
