...
tresult PLUGIN_API controller::initialize(FUnknown* in_context) {
	 ...
	 if (result != kResultOk) {
		...
		parameters.addParameter(STR("Sin Tone"), STR("%"), 0, 1., Steinberg::Vst::ParameterInfo::kCanAutomate,
					 parameter_id::sin_tone);
	 return kResultOk;
}
	 ...
	 if (auto param = parameters.getParameter(parameter_id::sin_tone))
